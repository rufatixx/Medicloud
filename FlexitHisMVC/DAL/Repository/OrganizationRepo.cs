    ...
    public class OrganizationRepo
    {
        ...
        public List<Organization> GetOrganizationListByUser(int userID)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                ...
                connection.Open();
                using (MySqlCommand organizationCom = new MySqlCommand("SELECT *,(select name from organizations where id = a.organizationID) as hName FROM user_organization_rel a where a.userID = @userID GROUP BY a.organizationID;", connection))
                {
