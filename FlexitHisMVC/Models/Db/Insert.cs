...
namespace FlexitHis_API.Models.Db
{
    public class Insert
    {
        ...
        public Insert(IConfiguration configuration, IWebHostEnvironment hostingEnvironment)
        {
        ...
        public ResponseStruct<int> InsertOrder(int userID, AddPatientStruct newPatient)
        {
...
WHERE NOT EXISTS 
  (SELECT * FROM patients WHERE name=@name and surname=@surname and father= @father and hospitalID = @hospitalID);", connection))
                    ...
                    if (lastID > 0)
                    {
                      ...
                      Values (@requestTypeID, @userID,@patientID,@hospitalID,@depID,@serviceID,@docID,@priceGroupID,@note,@referDocID)", connection))
